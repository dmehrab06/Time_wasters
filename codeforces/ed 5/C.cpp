...
void dfs(int paridx, int curx, int cury){
    vis[curx][cury]=true;
    par[curx][cury]=paridx;
    compsize++;
    for(int i=0;i<4;++i){
        if(curx+dx[i]>=0 && curx+dx[i]<n && cury+dy[i]>=0 && cury+dy[i]<m && !vis[curx+dx[i]][cury+dy[i]] && lab[curx+dx[i]][cury+dy[i]]=='.'){
            dfs(paridx,curx+dx[i],cury+dy[i]);
        }
    }
    int a=compsize%10;
    siz[curx][cury]=a;
        ...
        for(int j=0;j<m;++j){
            if(lab[i][j]=='.'){
                ...
                for(int k=0;k<4;++k){
                    ...
                    //int ans=0;
                    if(tx>=0 && tx<n && ty>=0 && ty<m && lab[tx][ty]=='.'){
                        int f=1;
                        for(int ii=0;ii<4;++ii){
                            if(par[tx][ty]==tpar[ii]){
                                f=0;
                                break;
                            }
                        }
