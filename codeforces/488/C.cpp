...
vector<PDD>paralsquare;
vector<PDD>alignsquare;

vector<line>lparal;
vector<line>lalign;

PDD rotpoint(PDD p){
    double x = p.first; double y = p.second;
    double sq2 = sqrt(2.0);
    double xx = (x-y)/sq2; double yy = (x+y)/sq2;
    return make_pair(xx,yy);
}

bool isinside(){
    double minx = 1000.00, maxx = -1000.00;
    FREP(i,0,3)minx = min(minx,paralsquare[i].first);
    FREP(i,0,3)maxx = max(maxx,paralsquare[i].first);

    double miny = 1000.00, maxy = -1000.00;
    FREP(i,0,3)miny = min(miny,paralsquare[i].second);
    FREP(i,0,3)maxy = max(maxy,paralsquare[i].second);
    int f = 1;
    FREP(i,0,3){
        double cx = alignsquare[i].first;
        double cy = alignsquare[i].second;
        if(INRANGED(cx,minx,maxx) && INRANGED(cy,miny,maxy)){
...
int main(){
    ...
    FREP(i,0,3){
        PDD p = paralsquare[i];
        PDD pp = paralsquare[(i+1)%4];
        line l(p,pp);
        lparal.PB(l);
    }
    FREP(i,0,3){
        PDD p = alignsquare[i];
        PDD pp = alignsquare[(i+1)%4];
        line l(p,pp);
        lalign.PB(l);
    }
    FREP(i,0,3){
        ...
        FREP(j,0,3){
            line l2 = lalign[j];
            PDD pp = l1.linesolve(l2);
            //cout<<"1st line\n";
            //l1.print();
            //cout<<"2nd line\n";
            //l2.print();
            PDD myp = l1.getpoint(pp.first);
            PDD myp2 = l2.getpoint(pp.second);
            //cout<<"intersection1 is "<<myp.first<<" "<<myp.second<<"\n";
            //cout<<"intersection2 is "<<myp2.first<<" "<<myp2.second<<"\n";
            if(l1.ispointonline(myp) && l2.ispointonline(myp2)){
                //cout<<i<<" intersect with "<<j<<"\n";
                cout<<"YES\n";
                return 0;
            }
        }
    }
    //checking whether aligned completely inside axis parallel
    if(isinside()){
        //cout<<"align inside parallel\n";
        cout<<"YES\n";
        return 0;
    }
    FREP(i,0,3)swap(alignsquare[i],paralsquare[i]);
    FREP(i,0,3){
        paralsquare[i] = rotpoint(paralsquare[i]);
        alignsquare[i] = rotpoint(alignsquare[i]);
    }
    if(isinside()){
        //cout<<"parallel inside align\n";
        cout<<"YES\n";
        return 0;
    }
    else{
