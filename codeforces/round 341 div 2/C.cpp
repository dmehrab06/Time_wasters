...
int main(){
    ...
    vector<long long int>difs;
    vector<long long int>totaldiv;
    int sz=n;
    while(n--){
        long long int l,r;
        cin>>l>>r;
        ls.push_back(l);
        rs.push_back(r);
        difs.push_back(r-l+1);
        long long int ldiv,rdiv;
        if(l%p==0){
            ldiv=l;
        }
        else{
            long long int mod=l%p;
            ldiv=l-mod+p;
        }
        if(r%p==0){
            rdiv=r;
            ...
            long long int mod=r%p;
            rdiv=r-mod;
        }
        long long int tot=(rdiv-ldiv)/p+1;
        if(tot<=0)totaldiv.push_back(0);
        else{
            totaldiv.push_back(tot);
        }
    ...
    for(int i=0;i<sz-1;++i){
        totalval=totalval+2000.0*(double)(totaldiv[i]*difs[i+1]+(difs[i]-totaldiv[i])*totaldiv[i+1])/( (double) (difs[i]*difs[i+1]));
    }
    totalval=totalval+2000.0*(double)(totaldiv[sz-1]*difs[0]+(difs[sz-1]-totaldiv[sz-1])*totaldiv[0])/( (double) (difs[sz-1]*difs[0]));
    printf("%.8lf\n",totalval);
}
