...
vector< vector <int> >edges;
vector<bool>visited;
void dfshedgehog(int depth, int cur){
    ...
    for(int i=0;i<l;++i){
        int now=edges[cur][i];
        if(!visited[now] && now>cur){
            dfshedgehog(depth+1,now);
        }
    }
}
...
long long int longestchain(int n){
    if(dp[n]!=-1)return dp[n];
    int l=edges[n].size();
    long long int mx=0;
    for(int i=0;i<l;++i){
        if(edges[n][i]<n){
            mx=max(mx,1+longestchain(edges[n][i]));
        }
    }
    dp[n]=mx;
    return dp[n];
...
int main(){
    ...
    long long int mx=-1;
    for(int i=n;i>=1;--i){
        mx=max(mx,(longestchain(i)+1)*degree[i]);
    }
    cout<<mx<<"\n";
}
