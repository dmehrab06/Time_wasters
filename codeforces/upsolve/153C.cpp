...
//int degree[100005];
vector<int>allnum;
int bs1(int val, int d, int low, int high, int sz){
    int mid=(low+high)/2;
    int dif=allnum[mid]-val;
    if(dif<=d){
        if(mid+1<sz && allnum[mid+1]-val<=d){
            return bs1(val,d,mid+1,high,sz);
        }
        else{
            return mid;
        }
...
int main(){
    ...
    for(int i=0;i<sz-2;++i){
        long long int thisway=0;
        if(allnum[i+2]-allnum[i]<=d){
            int idx=bs1(allnum[i],d,i,sz-1,sz);
            thisway=(long long  int)(idx-i)*(idx-i-1);
            thisway/=2;
