...
#define mp make_pair
pair<InfInt,InfInt> DP[55] ;
int F[55] ;
int n,p ;
void init(){
    memset(F,-1,sizeof(F));
}
InfInt __gcd(InfInt a, InfInt b){
    if(a==0)return b;
    return __gcd(b%a,a);
}

pair<InfInt,InfInt> call(int x){

    if(x==0) return mp((InfInt)0,(InfInt)1) ;
    if(F[x]!=-1) return DP[x] ;
    pair<InfInt,InfInt> prev = call(x-1) ;
    InfInt x1 = prev.first*x*(100-p)+prev.second*100*n ;
    InfInt x2 = prev.second*(100*x-x*p) ;
    InfInt gcd = __gcd(x1,x2) ;
    x1/=gcd ;
    x2/=gcd ;
    DP[x] = mp(x1,x2) ;
    F[x]=1;
