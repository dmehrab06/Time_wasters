...
LLI solve(int dir, int uneaten, int cur){
    ...
    if(dir==0){
        ...
        if(uneaten>0){
            ...
            int ret1 = (N-cur+uneaten)*dif+solve(1,cur+1,uneaten);
            if(ret1<ret){
                ret = ret1;
        ...
        if(cur<((int)locs.size()-2)){
            ...
            int ret1 = (N-cur+uneaten)*dif+solve(0,uneaten,cur+1);
            if(ret1<ret){
                ret = ret1;
        ...
        if(uneaten<((int)locs.size()-1)){
            ...
            int ret1 = (N+cur-uneaten)*dif+solve(0,cur-1,uneaten);
            if(ret1<ret){
                ret = ret1;
        ...
        if(cur>1){
            ...
            int ret1 = (N+cur-uneaten)*dif+solve(1,uneaten,cur-1);
            if(ret1<ret){
                ret = ret1;
...
int main(){
    ...
    LLI ans1 = abs(l-locs[pos].first)*(N)+solve(0,pos-1,pos);
    //cout<<"ans1 is "<<ans1<<"\n";
    LLI ans2 = abs(l-locs[pos-1].first)*(N)+solve(1,pos,pos-1);
    if(l>locs[locs.size()-2].first){
        LLI tot = 0;
        //LLI cum1 = 0;
        //LLI cum2 = 0;
        int t1 = l;
        RFREP(i,locs.size()-2,1){
            int tt = abs(t1-locs[i].first);
            tot+=tt;
            //t1  = locs[i].first;
        }
        cout<<tot<<"\n";
    }
    else if(l<locs[1].first){
        LLI tot = 0;
        //LLI cum1 = 0;
        //LLI cum2 = 0;
        int t1 = l;
        FREP(i,1,locs.size()-2){
            int tt = abs(t1-locs[i].first);
            //cum1+=tt;
            //cum2+=cum1;
            tot+=tt;
