typedef std::vector<char> matrixRow;
typedef std::vector<matrixRow> matrix;

matrix fractal(int n) {
  int size = (1 << n) - 1;
  ...
  struct Helper {
    ...
    Helper(int size) : 
      result(matrix(size, matrixRow(size, 0))),
      resultTmp(matrix(size, matrixRow(size, 0))) {}
  ...
  Helper h = Helper(size);
  h.draw(n, 0, 0, 0);
  matrix result = h.result;
  matrix answer(size + 1, matrixRow((size + 1) * 2 - 1, 0));
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      int cell = result[i][j];
      if ((cell >> 2) % 2 == 1) {
  ...
  for (int i = 0; i < size + 1; i++) {
    for (int j = 0; j < (size + 1) * 2 - 1; j++) {
      ...
      if (answer[i][j] == ' ' && j > 0 && 
          answer[i][j - 1] == '_' && j + 1 < answer[i].size() && 
          answer[i][j + 1] == '_') {
        answer[i][j] = '_';
      }
    }
