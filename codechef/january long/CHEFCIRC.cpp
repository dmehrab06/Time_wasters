...
double getradcolinear(PDD p1, PDD p2, PDD p3){
    return max(dis(p1,p2),max(dis(p2,p3),dis(p1,p3)));
}

PDD getcentrecolinear(PDD p1, PDD p2, PDD p3){
    double maxdis = dis(p1,p2);
    PDD centre = make_pair((p1.first+p2.first)/2.0,(p1.second+p2.second)/2.0);
    if(dis(p2,p3)>maxdis){
        maxdis=dis(p2,p3);
        centre=make_pair((p3.first+p2.first)/2.0,(p3.second+p2.second)/2.0);
    }
    if(dis(p1,p3)>maxdis){
        maxdis=dis(p1,p3);
        centre=make_pair((p3.first+p1.first)/2.0,(p3.second+p1.second)/2.0);
    }
    return centre;
}

pair< PDD, double >solve(PDD p1, PDD p2, PDD p3){

    if(iscolinear(p1,p2,p3)){
    //    cout<<"colinear\n";
        PDD cent=getcentrecolinear(p1,p2,p3);
        cent.first=cent.first*(-1.0);
        cent.second=cent.second*(-1.0);
        double rad=getradcolinear(p1,p2,p3)/2.0;
        double C=(cent.first*cent.first)+(cent.second*cent.second)-rad*rad;
        //cout<<"colinear\n";
        //cout<<"cent "<<cent.first<<" "<<cent.second<<"\n";
        //cout<<"rad "<<rad<<"\n";
        return make_pair(cent,C);
    }
...
int main(){
    ...
    if(m==1){
        ...
        FREP(i,1,n){
            FREP(j,(i+1),n){
                double dd=dis(points[i],points[j]);
                if(mnn2<0.00 || dd<mnn2){
                    mnn2=dd;
                }
            }
        }
