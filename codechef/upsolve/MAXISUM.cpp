...
int main(){
    ...
    while(t--){
        ...
        for(int i=1;i<=n;++i){
            interact[i]=A[i]*B[i];
            /*if(interact[i]>maxval){
                maxidx=i;
                maxval=interact[i];
            }*/
            thissum+=interact[i];
        }
        long long int mxsum=thissum;
        long long int KK=K;
        for(int maxidx=1;maxidx<=n;++maxidx){
        long long int aa=B[maxidx]*KK;
        long long int bb=B[maxidx]*(-KK);
       // long long int cc=A[maxidx]*(B[maxidx]+K);
        //long long int dd=A[maxidx]*(B[maxidx]-K);
        //long long int mx=max(aa,bb);
        long long int cursuma=thissum+aa;
        long long int cursumb=thissum+bb;
        mxsum=max(mxsum,max(cursuma,cursumb));
        }
        printf("%lld\n",mxsum);
    }
    return 0;
