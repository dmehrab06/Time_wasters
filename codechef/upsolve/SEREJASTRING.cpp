...
int main(){
  ...
  vector<string>allstr2;
  for (map< string,int>::iterator it=allstr.begin(); it!=allstr.end(); ++it){
        allstr2.push_back(it->first);
   }
  int setl=allstr2.size();
  int c=0;
  for(int i=0;i<setl;++i){
    for(int j=i+1;j<setl;++j){
        string aa=allstr2[i];
        string bb=allstr2[j];
        if(issimilar(aa,bb)){c++;
            cout<<aa<<" and "<<bb<<" are similar\n";
        }
    }
  }
  printf("%d\n",c);
