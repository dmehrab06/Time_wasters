...
long long int twopows[72];
long long int twop(int m){
    long long int s=1;
    for(int i=1;i<=m;++i){
        s*=2;
    }
...
long long int threepows[72];
long long int threep(int m){
    long long int s=1;
    for(int i=1;i<=m;++i){
        s*=3;
    }
...
int main(){
   ...
   // printf("esechi\n");
    ...
    for(int i=twop;i<=65;++i){
        for(int j=threep;j<=40;++j){
            if( (long long int) (i+1)*(j+1)==A){
                Ms.push_back(twopows[i]*threepows[j]);
            }
        }
    }
    ...
    for(int i=twop;i<=65;++i){
        for(int j=threep;j<=40;++j){
            if( (long long int) (i+1)*(j+1)==B){
                Ns.push_back(twopows[i]*threepows[j]);
            }
        }
    }
    long long int M,N;
    ...
    int mstarti;
    for(int i=0;i<lm;++i){
        if(Ms[i]>0 && Ms[i]<=1000000000000000000){
            mstarti=i;
            break;
        }
    }
    ...
    int nstarti;
    for(int i=0;i<ln;++i){
        if(Ns[i]>0 && Ns[i]<=1000000000000000000){
            nstarti=i;
            break;
        }
    }
    int flag=0;
    for(int i=mstarti;i<lm;++i){
        for(int j=nstarti;j<ln;++j){
            if(gcd(Ms[i],Ns[j])==g){
                M=Ms[i];
                N=Ns[j];
                flag=1;
