...
int main(){
    ...
    for(int i=2;i<n;){
        ...
        //if leftswap;
        if(i>=3){
            lpast1=abs(arr[i-1]-arr[i-2]);
            lnext1=abs(arr[i]-arr[i-2]);
        }
        lpast2=abs(arr[i]-arr[i+1]);
        lnext2=abs(arr[i-1]-arr[i+1]);
        int profit2=lnext1+lnext2-lpast1-lpast2;
        int rpast1=0;
        int rpast2=0;
        int rnext1=0;
        int rnext2=0;
        if(i<=n-2){
            rpast2=abs(arr[i+1]-arr[i+2]);
            rnext2=abs(arr[i]-arr[i+2]);
        }
        rpast1=abs(arr[i]-arr[i-1]);
        rnext1=abs(arr[i+1]-arr[i-1]);
        int profit3=rnext1+rnext2-rpast1-rpast2;
        int minprofit;
        if(profit3<profit2){
            minprofit=3;
            if(profit3<profit1){
