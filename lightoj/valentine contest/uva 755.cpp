...
//int arr[503][503];
int telemaps[26]={50,50,50,51,51,51,52,52,52,53,53,53,54,54,54,55,81,55,55,56,56,56,57,57,57,90};
int main(){
    int t;
    cin>>t;
    while(t--){
        ...
        while(data--){
            ...
            string right="";
            for(int i=0;i<3;++i){
                if('A'<=tele[i] && tele[i]<='Z'){
                    left+=telemaps[tele[i]-'A'];
                }
                else{
                    left+=tele[i];
                }
            ...
            res+="-";
            res+=right;
            if(telephones.find(res)==telephones.end()){
                telephones[res]=1;
            }
            else{
                telephones[res]++;
        ...
        if(!dupflag){
            ...
            vector<myitem>alltele;
            typedef map<string,int>::iterator it_type;
            for(it_type it=telephones.begin();it!=telephones.end();++it){
                myitem tel(it->first,it->second);
                alltele.push_back(tel);
            }
            sort(alltele.begin(),alltele.end(),mycompare);
            int l=alltele.size();
            for(int i=0;i<l;++i){
                cout<<alltele[i].s<<" "<<alltele[i].freq<<"\n";
            }
        }
    }
    return 0;
