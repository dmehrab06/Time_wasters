...
int query(int i, int j, int b, int e, int node){
    ...
    int right=left+1;
    int mid=(b+e)/2;
    int min1=query(i,j,b,mid,left);
    int min2=query(i,j,mid+1,e,right);
    return min1+min2;
}
    ...
    while(t--){
        ...
        for(int i=1;i<=Q;++i){
            ...
            if(type==1){
                int sack;
                scanf("%d",&sack);
                int togive=query(sack+1,sack+1,1,N,1);
                printf("%d\n",togive);
                update(sack+1,sack+1,1,N,1,-togive);
            }
            else if(type==2){
                int sack,amount;
                scanf("%d%d",&sack,&amount);
                update(sack+1,sack+1,1,N,1,amount);
            }
            else if(type==3){
                int i,j;
                scanf("%d%d",&i,&j);
                int tot=query(i+1,j+1,1,N,1);
                printf("%d\n",tot);
            }
        }
