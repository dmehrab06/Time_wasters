...
int segtreemax[40005];
int segtreemin[40005];
int arr[10005];

void initmax(int b, int e, int node){
    if(b==e){
        segtreemax[node]=arr[b];
  ...
  //  printf("in node %d, range %d to %d\n",node,b,e);
    ...
    int mid=(b+e)>>1;
    initmax(b,mid,left);
    initmax(mid+1,e,right);
    segtreemax[node]=max(segtreemax[left],segtreemax[right]);
    }
}

int querymax(int i, int j, int b, int e, int node){
    if(b>=i && e<=j){
    ...
    int right=left+1;
    int mid=(b+e)>>1;
    int min1=querymax(i,j,b,mid,left);
    int min2=querymax(i,j,mid+1,e,right);
    return max(min1,min2);
}

void initmin(int b, int e, int node){
    if(b==e){
        segtreemin[node]=arr[b];
  ...
  //  printf("in node %d, range %d to %d\n",node,b,e);
    ...
    int mid=(b+e)>>1;
    initmin(b,mid,left);
    initmin(mid+1,e,right);
    segtreemin[node]=min(segtreemin[left],segtreemin[right]);
    }
}

int querymin(int i, int j, int b, int e, int node){
    if(b>=i && e<=j){
    ...
    int right=left+1;
    int mid=(b+e)>>1;
    int min1=querymin(i,j,b,mid,left);
    int min2=querymin(i,j,mid+1,e,right);
...
int main(){
    ...
    while(t--){
        ...
        for(int i=1;i+d-1<=N;++i){
            int maxx=querymax(i,i+d-1,1,N,1);
            int minx=querymin(i,i+d-1,1,N,1);
            maxdif=max(maxdif,maxx-minx);
        }
        printf("%d\n",maxdif);
