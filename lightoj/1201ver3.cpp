...
void makeg2(){
    ...
    set<int>aa;
    map<int,int>aamap;
    FREP(i,0,asz-1)aa.insert(As[i]);
    FREP(i,0,asz-1)aamap[As[i]]=i;
    FREP(i,0,bsz-1)addedge(S,i+1,1,0);
    FREP(i,0,asz-1)addedge(bsz+1+i,T,1,0);
    FREP(i,0,bsz-1){
        int cur = Bs[i];
        FREP(j,0,g[cur].size()-1){
            int v = g[cur][j];
            if(aa.find(v)!=aa.end()){
                addedge(i+1,aamap[v]+bsz+1,1,0);
            }
        }
    }
}
...
int main()
{
    ...
    FREP(cs,1,t){
        ...
        int sing = takeinp(n,m);
        LLI ans1 = (LLI)n-solve(n);
        printf("Case %d: %lld\n",cs,ans1);
    }
    return 0;
}
