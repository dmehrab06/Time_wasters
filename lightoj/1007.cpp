...
int main(){
    ...
    while(t--){
        int battle;
        for(int i=1;i<20005;++i){visited[i]=0;appears[i]=0;}
        vector<int>line(0);
        for(int i=1;i<20005;++i)edges.push_back(line);
        scanf("%d",&battle);
        while(battle--){
            int u,v;
            scanf("%d %d",&u,&v);
       ...
       // cout<<node<<"\n";
        //for(int i=0;i<allnodes.size();++i)cout<<allnodes[i]<<"\n";
       // cout<<"hello\n";
        int getmaxteam=BFS(allnodes, node);
        //cout<<"hello2\n";
        printf("Case %d: %d\n",cs++,getmaxteam);
        int nn=edges.size();
        for(int i=0;i<nn;++i){
