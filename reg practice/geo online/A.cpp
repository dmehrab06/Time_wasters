...
P2D calcD(P2D A, P2D B, P2D C){
        ...
        double ang2=ang(BC,CA);
        double rot1=ang1/3.0;
        double rot2=ang2/3.0;
        P2D newC=rotate_point(B.x,B.y,rot1,C);
        P2D newB=rotate_point(C.x,C.y,-rot2,B);
        P2D newC2=rotate_point(B.x,B.y,-rot1,C);
        P2D newB2=rotate_point(C.x,C.y,rot2,B);
        Line l1(B,newC);
        Line l2(C,newB);
        P2D I1;
        bool d1=areintersect(l1,l2,I1);
        Line l3(B,newC2);
        Line l4(C,newB2);
        P2D I2;
        bool d2=areintersect(l3,l4,I2);

        if(d1 && isinside(A,B,C,I1)){
            return I1;
        }
        else if(d2 && isinside(A,B,C,I2)){
            return I2;
        }
}
...
P2D calcE(P2D A, P2D B, P2D C){
        ...
        double ang2=ang(AB,CA);
        double rot1=ang1/3.0;
        double rot2=ang2/3.0;
        P2D newC=rotate_point(A.x,A.y,rot1,C);
        P2D newA=rotate_point(C.x,C.y,-rot2,A);
        P2D newC2=rotate_point(A.x,A.y,-rot1,C);
        P2D newA2=rotate_point(C.x,C.y,rot2,A);
        Line l1(A,newC);
        Line l2(C,newA);
        P2D I1;
        bool d1=areintersect(l1,l2,I1);
...
P2D calcF(P2D A, P2D B, P2D C){
        ...
        double ang2=ang(BC,AB);
        cout<<ang1<<" "<<ang2<<"\n";
        double rot1=ang1/3.0;
        double rot2=ang2/3.0;
        P2D newB=rotate_point(A.x,A.y,rot1,B);
        P2D newA=rotate_point(B.x,B.y,-rot2,A);
        P2D newB2=rotate_point(A.x,A.y,-rot1,B);
        P2D newA2=rotate_point(B.x,B.y,rot2,A);
        Line l1(A,newB);
        Line l2(B,newA);
        P2D I1;
        bool d1=areintersect(l1,l2,I1);
