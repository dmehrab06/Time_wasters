...
long long int getval(long long int idx){
    if(idx==-1){
        return 0;
    }
    if(idx==0){
    ...
    long long int ext=idx/4;
    long long int last4=ext*4;
    //find sum until last4
    long long int mod1val=ext;
    long long int mod2val=2*ext*(ext+1)-ext;
    long long int lastval=4*ext-1;
    long long int mod0val=mod2val+ext;
    long long int partans=mod1val+mod2val+mod0val;
    int extramod1=last4+1;
    if(extramod1<=idx){
        partans+=1;
    }
    int extramod2=extramod1+1;
    if(extramod2<=idx){
        partans+=(lastval+4);
    }
    return partans;
...
int main(){
    ...
    while(t--){
        long long int l,r;
        scanf("%lld %lld",&l,&r);
        long long int ans1=getval(r);
        long long int ans2=getval(l-1);
        printf("%lld\n",ans1-ans2);
    }
    return 0;
}
