...
long long int paldigit[21];
long long int smallest[21];
char palindrome[31];
int main(){
    long long int smallstart=1;
    for(int i=1;i<=18;++i){
        smallest[i]=smallstart;
        smallstart*=10;
    }
    long long int start=9;
    ...
    paldigit[0]=0;
    for(int i=1;i<=20;++i){
        paldigit[i]=paldigit[i-1]+paldigit[i];
    }
    int t;
    scanf("%d ",&t);
    int cs=1;
    while(t--){
        scanf("%s",palindrome);
        int dig=strlen(palindrome);
        long long int untillast=paldigit[dig-1];
        int tocheck=dig/2;
        if(dig%2)tocheck++;
        long long int ss=0;
        int added=0;
        for(int i=0;i<tocheck;++i){
            ss=ss*10+(palindrome[i]-'0');
            added++;
        }
        long long int smlest=smallest[added];
        long long int dif=ss-smlest+1;
        long long int ans=dif+untillast;
        printf("Case %d: %lld\n",cs++,ans);
