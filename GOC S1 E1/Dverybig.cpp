...
frac add(frac a, frac b){
    InfInt losagu=a.hor/(gcd(a.hor,b.hor));
    losagu=losagu*b.hor;
    InfInt anslob=(losagu/a.hor)*a.lob+(losagu/b.hor)*b.lob;
...
int main(){
    ...
    while(t--){
        ...
        frac sum(0,1);
        InfInt nn=n;
        InfInt hor=(nn*(nn-1))/2;
        InfInt curp=1;
        FREP(i,0,31){
            int zzz=countzincol(n,i);
            InfInt z=zzz;
            InfInt zz=(z*(z-1))/2;
            InfInt curlob=hor-zz;
            curlob=curlob*curp;
        ...
        InfInt gg=gcd(sum.lob,sum.hor);
        sum.lob/=gg;
        sum.hor/=gg;
        cout<<sum.lob<<"/"<<sum.hor<<"\n";
    }
    return 0;
}
