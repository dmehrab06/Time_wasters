...
bool isallzero(int x1, int y1, int x2, int y2, int n, int m){
    int s1 = getcnt(x2,y2,0,n,m);
    int s2 = getcnt(x2,y1-1,0,n,m);
    int s3 = getcnt(x1-1,y2,0,n,m);
    int s4 = getcnt(x1-1,y1-1,0,n,m);
    int tot = s1-s2-s3+s4;
    int area = (x2-x1+1)*(y2-y1+1);
    if(tot==area)return true;
    return false;
}

bool isvalid(int x1, int y1, int x2, int y2, int n, int m){
    int oddfound = 0;
    FREP(i,0,9){
...
pair<int, pair<PII,PII> >solvedp(int x1, int y1, int x2, int y2, int n ,int m){
    ...
    if(isallzero(x1,y1,x2,y2,n,m)){
        ...
        if(ar1>ar2){
            ...
            pair<int,pair<PII,PII> >ans1 = solvedp(x1+1,y1,x2,y2,n,m);
            if(ans1.first==ar2){areamap[mapentry]=ans1;return ans1;}
            pair<int,pair<PII,PII> >ans2 = solvedp(x1,y1,x2-1,y2,n,m);
            if(ans2.first==ar2){areamap[mapentry]=ans2;return ans2;}
            pair<int,pair<PII,PII> >ans3 = solvedp(x1,y1+1,x2,y2,n,m);
            pair<int,pair<PII,PII> >ans4 = solvedp(x1,y1,x2,y2-1,n,m);
            pair<int,pair<PII,PII> >ans = findmax(ans1,ans2);
            ans = findmax(ans,ans3); ans = findmax(ans,ans4);
