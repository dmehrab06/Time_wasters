...
void build(int node, int b, int e){
    if(b==e){
        cout<<"in node "<<node<<" begin: "<<b<<" end: "<<e<<" contains "<<segtree[node].len<<" "<<segtree[node].o<<" "<<segtree[node].c<<"\n";
        segtree[node].len=0;
        if(doorshifted[b]=='O'){segtree[node].o=0;segtree[node].c=1;}
        else {segtree[node].c=0; segtree[node].o=1;}
        cout<<"after changing in node "<<node<<" begin: "<<b<<" end: "<<e<<" contains "<<segtree[node].len<<" "<<segtree[node].o<<" "<<segtree[node].c<<"\n";
        return;
    }
    int mid=(b+e)/2;
    int l=node<<1; int r=node+1;
    ...
    segtree[node].len=segtree[l].len+segtree[r].len+t;
...
nodestate query(int node, int b, int e, int lft, int rgt){
    ...
    nodestate mynode(lans.len+rans.len+t,lans.o+rans.o-t,lans.c+rans.c-t);
    return mynode;
}
