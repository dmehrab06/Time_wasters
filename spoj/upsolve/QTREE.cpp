...
void init(int l, int r, int curnode){
    ...
    init(l,mid,lnode);
    init(mid+1,r,rnode);
    segtree[curnode]=max(segtree[lnode],segtree[rnode]);
}

void update(int l, int r, int curnode, int idx, int val){
    if(l>r || idx<l || idx>r){
        return;
    }
    if(l==r && l==idx){
...
int hldquery(int u, int v, int n){
    ...
    return max(tmp,nxtans);
}

int getans(int u, int v, int n){
    int lca=LCA(u,v,18);
   // printf("lca of %d and %d is %d\n",u,v,lca);
    int ans1=hldquery(u,lca,n);
    int ans2=hldquery(v,lca,n);
