...
int findnotlcs(int i, int j){
    ...
    if(kval[i][j]==-1){
        ...
        int kk=kval[i][j];
        int mx1=1+findnotlcs(i-1,kk-1);
        int mx2=findnotlcs(i-1,j);
        ls[i][j]=min(mx1,mx2);
        return ls[i][j];
    }
}
int main(){
    ...
    vector<int>line;
    for(int i=0;i<279;++i){
		charpos.push_back(line);

    }
    for(int i=0;i<lb;++i){
    	charpos[b[i]].push_back(i+1);
    }
   	int la=a.size();
   	...
   	/*for(int i='a';i<='z';++i){
        int l=charpos[i].size();
        printf("%c: ",i);
        for(int j=0;j<l;++j){
            printf("%d ",charpos[i][j]);
        }
        printf("\n");
   	}*/
   	for(int i=0;i<la;++i){
   		int curcar=a[i];
   		int sz=charpos[curcar].size();
   		int setpos=sz-1;
   		for(int j=lb-1;j>=0;--j){
   			if(setpos>=0){
	   			if((j+1)>=charpos[curcar][setpos]){
	   			 	kval[i+1][j+1]=charpos[curcar][setpos];
	   			}
				else{
					setpos--;
					if(setpos>=0){
