    ...
    while(scanf("%d",&n)==1){
        ...
        while(tot--){
            ...
            if(nodes.find(src)==nodes.end()){
                nodes[src]=totnode;
                u=totnode;
                nodenames[totnode]=src;
                totnode++;
            }
            else{
                u=nodes[src];
            }
            if(nodes.find(des)==nodes.end()){
                nodes[des]=totnode;
                v=totnode;
                nodenames[totnode]=des;
                totnode++;
            }
            else{
                v=nodes[des];
        ...
        if(nodes.find(from)==nodes.end()){
        ...
        else if(nodes.find(to)==nodes.end()){
            if(f==0){
                    printf("\n");
                }
            printf("No route\n");
            ...
            int s=nodes[from];
            int d=nodes[to];
            int pathexists=BFS(s,d,totnode-1);
            if(pathexists==-1){
                if(f==0){
                    printf("\n");
                }
                ...
                for(int i=pathsize-1;i>0;--i){
                    cout<<nodenames[path[i]]<<" "<<nodenames[path[i-1]]<<"\n";
                }
            }
