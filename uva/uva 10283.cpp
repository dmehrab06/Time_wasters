...
int main(){
    ...
    while(scanf("%lf %lf",&R,&N)==2){
        ...
        /*
            ...
            so,
                substituting x
                r=Rsintheta/(1+sintheta);
        */
        double sinang=sin(degtorad(theta));
        double r=(R*sinang)/(1+sinang);
        double smallcirclearea=pi*r*r;
        double arcangle=90.0-theta;
        double arcarea=(smallcirclearea/360.0)*arcangle;
        double trianglearea=0.5*r*(R-r)*cos(degtorad(theta)); //r=lombo, bakituku hocche vumi, otivuj ar angle theke derived
        double bluearea=trianglearea-arcarea;
        bluearea=bluearea*2.0*N;
        double greenarea=(pi*R*R)-((smallcirclearea*N)+bluearea);
