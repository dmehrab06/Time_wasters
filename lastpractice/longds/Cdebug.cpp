    ...
    FREP(i,1,q){
        ...
        if(id==0){
            ...
            if(empstate[id]%2){
                ...
                if(hooks.find(l)!=hooks.end())oldl = lftelem2.first;
                if(hooks.find(r)!=hooks.end())oldr = rgtelem2.second;
                PII oldelem = make_pair(oldl,oldr);
                if(hooks2.find(lftelem2)!=hooks2.end()){hooks2.erase(lftelem2);hooks.erase(l);}
                if(hooks2.find(rgtelem2)!=hooks2.end()){hooks2.erase(rgtelem2);hooks.erase(r);}
        ...
        set<pair<int,PII>>::iterator it2;
        cout<<"hooks contents\n";
        for(it2 = hooks.begin();it2!=hooks.end();++it2){
            cout<<(*it2).second.second<<" "<<-((*it2).second.first)<<"\n";
        }
        cout<<"\n\n";
    }
}
